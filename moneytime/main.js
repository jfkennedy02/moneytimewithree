...
function initDynamicRouter() {
    ...
    if (isLocalFile) {
        console.warn('Money Time With Ree Router: Local preview mode. Fallback to native loading due to file:// protocol CORS rules.');
        
        // Native fallbacks for anchor scroll clicks
...
function loadPageDynamically(url, pushState = true) {
    ...
    fetch(url)
        ...
        .then(html => {
            // Wait 300ms for overlay swipe cover animation to cover the page
            setTimeout(() => {
